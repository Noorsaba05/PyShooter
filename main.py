...
pygame.display.set_caption('PyShooter - Noorsaba')
	...
	for event in pygame.event.get():
		...
		if event.type == pygame.KEYDOWN:
			...
			if event.key == pygame.K_d or event.key == pygame.K_RIGHT:	# key "D" or "Right arror" => move right
				moving_right = True
			if event.key == pygame.K_SPACE or event.key == pygame.K_z :	# key " " => shoot
				shoot = True
			if event.key == pygame.K_q or event.key == pygame.K_h:	# key "Q" or "G" => through grenade
				grenade = True
